...
public final class SensitiveInfoUtils {
    public static void main(String[] args) {
        String name = "张大炮";
        System.out.println(StringUtils.left(name, 1));
        name = StringUtils.rightPad(StringUtils.left(name, 1), StringUtils.length(name), "*");
        System.out.println(name);
        name  = "深圳赢时胜股份有限公司";
        System.out.println(sub(name,2,6));
    }


    //测试
    public static String sub(String fullName,int begin,int end){
        if (fullName==null || fullName.length() < end){
            return fullName;
        }
        String prefix = StringUtils.rightPad(StringUtils.left(fullName, begin), StringUtils.length(fullName)-end, "*");
        String suffix = StringUtils.right(fullName, end);
